...
class Parser
  ...
  def initialize(language_name, &block)
    ...
    @reserved = ["end", "endwhile", "endfor", "endif", "input", "print", "printn", "int", "float",
  "bool", "list", "map", "nothing", "function", "string"]
    instance_eval(&block)
  end
  ...
  def parse(string)
    ...
    if @pos != @tokens.size
      ...
      if @reserved.include?(@tokens[@max_pos])
        raise ReservedNameError, "ReservedNameError, '#{@tokens[@max_pos]}' is a reserved word"
      end
